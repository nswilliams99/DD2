    ...
    const generateSitemap = () => {
      ...
      residentialTowns?.forEach(town => {
        sitemapUrls.push({
          ...
          priority: 0.7,
          changefreq: 'weekly',
          lastmod: town.updated_at?.split('T')[0]
        });
      });
...
${sitemapUrls.map(url => `  <url>
    ...
    <priority>${url.priority}</priority>
      ...
      sessionStorage.setItem('xmlSitemap', xmlSitemap);
      sessionStorage.setItem('sitemapUrls', JSON.stringify(sitemapUrls));
    };

    if (services && rolloffTowns && residentialTowns) {
      generateSitemap();
    }
